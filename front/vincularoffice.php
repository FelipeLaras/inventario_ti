...
<div class="container-fluid">
  ...
  <hr />
  <!-- /.container-fluid -->
  <form action="../inc/vincularoffice.php?id=<?= $_GET['id'] ?>" method="POST" autocomplete="off">
    ...
    <div class="col-md-4 py-4 input-group">
      <select name="equip" class="form-control" required>
        ...
        while ($equipamentos = $resultEquipamento->fetch_assoc()) {
          ...
          $queryoffice = "SELECT * FROM manager_office WHERE id_equipamento = " . $equipamentos['id_equipamento'] . "";
          $resultOffice = $conn->query($queryoffice);
          $office = $resultOffice->fetch_assoc();

          if (empty($office['id_equipamento'])) {
